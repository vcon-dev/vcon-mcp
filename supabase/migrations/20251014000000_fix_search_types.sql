-- Fix type mismatches in search functions
-- Issue: ts_rank_cd returns real (float4) but function signatures use float (which defaults to double precision in some contexts)
-- Solution: Cast return values to double precision for consistency
...
BEGIN
  ...
  kw AS (
    SELECT x.vcon_id, MAX(ts_rank_cd(x.tsv, plainto_tsquery('english', keyword_query), 32)::double precision) AS keyword_score
    FROM (
