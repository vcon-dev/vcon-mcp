...
async function main() {
  console.log('🚀 Starting batched tenant_id backfill...');
  console.log('   This script updates rows in small batches to avoid database timeouts.');
  ...
  for (const table of TABLES) {
    ...
    while (true) {
      try {
        ...
        const { data: count, error } = await supabase.rpc('backfill_tenant_id_batch', {
          p_table_name: table,
        ...
        // Small delay to let DB breathe / handle other requests
